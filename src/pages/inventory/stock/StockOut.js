...
const Index = () => {
    ...
    const handleStockAdd = async data => {
        setLoading(true)
        try {
            const response = await Requests.Inventory.Stock.StockOut(data)
            if (response && response.status === 201) {
                Toastify.Success("Stock Removed Successfully")
                history.push('/dashboard/inventory/stock/out-history')
            }
            setLoading(false)
        } catch (error) {
            if (error && error.response && error.response.status && error.response.status === 422) {
                Toastify.Error("Stock Can't Be Updated")
            } else {
            ...
            <Layout
                ...
                button={
                    <div className="print-hidden">
                        <Link to="/dashboard/inventory/stock/status">

...
const Index = () => {
    ...
    const handlePageChange = page => fetchDailyExpenseData(page)
    ...
    return (
        <div>
            <Layout
                page={t("dashboard / accounting / daily expense")}
                message={t("Who are using this system.")}
                container="container-fluid"
                button={
                    <Link to={`/dashboard/accounting/daily-expense/create`}>
                        <GrayButton
                            className="ml-2 mt-2 mt-sm-0"
                            // onClick={() => setCreateExpense({ ...isCreateExpense, show: true })}
                        >
                            <Text className="fs-13 mb-0"> <Plus size={13} /><span className='text-secondary ml-1'>{t('ADD NEW EXPENSE')}</span></Text>
                        </GrayButton>
                    </Link>
                }
            />
            <Main>
                ...
                <Container.Column>
                    <DataTable
                        ...
                        clearSearch={() => fetchDailyExpenseData(1)}
                    />
                </Container.Column>
            </Main>
